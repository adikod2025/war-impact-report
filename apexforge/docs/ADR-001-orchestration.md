...
|---|---|---|
| Intent layer issues roles, never trajectories | `MacroAction` has no waypoint/trajectory/heading field, and `__post_init__` rejects those keys in `params` | `test_contracts.py::test_macroaction_refuses_micromanagement_params`, `::test_macroaction_has_no_trajectory_fields_at_all` |
| No kinetic or effector logic | `Action.ALLOWED_TYPES` is a closed non-kinetic vocabulary; `MacroAction.FORBIDDEN_PARAM_KEYS` rejects `weapon`/`target_engagement`/`fire` | `::test_macroaction_refuses_kinetic_params`, `::test_action_vocabulary_is_closed_and_non_kinetic` |
| Human approval cannot be faked | `WorkflowEvent.human_approved` is True only for an attributed, approving `HumanDecision`; `operator_id` and `rationale` are mandatory at construction | `::test_bare_event_is_never_human_approved`, `::test_human_decision_requires_operator_identity` |
